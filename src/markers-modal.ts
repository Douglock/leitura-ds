...
export class MarkersModal extends Modal {
  ...
  onOpen(): void {
    ...
    const addRow = this.contentEl.createDiv({ cls: "flow-reader__marker-add" });
    ...
    const list = this.contentEl.createDiv({ cls: "flow-reader__marker-list" });
    if (!this.markers.length) list.createDiv({ cls: "flow-reader__search-empty", text: "Nenhum marcador adicional." });
    this.markers.forEach((marker) => {
      const row = list.createDiv({ cls: "flow-reader__marker" });
      const info = row.createDiv();
      info.createDiv({ cls: "flow-reader__marker-name", text: marker.name });
      info.createDiv({ cls: "flow-reader__marker-location", text: `${this.locationLabel} ${marker.position.chapterIndex + 1} · ${marker.position.word ?? "posição salva"}` });
